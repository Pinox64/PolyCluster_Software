...
pub fn getConfigFilePath(allocator: Allocator, envmap: std.process.EnvMap) ![]u8 {
    if (envmap.get("XDG_CONFIG_HOME")) |config_home_path| {
        return std.fmt.allocPrint(allocator, "{s}/PCluster/PCluster.zon", .{config_home_path});
    } else if (envmap.get("HOME")) |home_path| {
        return std.fmt.allocPrint(allocator, "{s}/.config/PCluster/PCluster.zon", .{home_path});
